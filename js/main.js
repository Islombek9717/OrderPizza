...
for (var i = 0; i < 3; i++){
    ...
    sizeLabel.setAttribute('for', 'flexRadioDefault'+i);
    ...
    getSize.setAttribute('id', 'flexRadioDefault'+i);
...
for (var i = 0; i < 3; i++){
    ...
    getType.setAttribute('name', 'meat');
...
for (var i = 0; i < 3; i++){
    ...
    getSous.setAttribute('name', 'sous');
...
form.addEventListener('click', function (e) {
    ...
    if (name === 'size' && !result.map(r=> r.value).includes(value)) {
        var data = {
            name:name, value:value
        }
        result = result.filter(el => el.name !== name)
        result.push(data)
        showResult.textContent = result.map(d=> d.value)+ 'li';
    ...
    if (value && name !== 'size' ) {
        ...
        showResult.textContent = result.map(d=> d.value)+'li pizza buyurtma qilmoqchimisiz?';
    }
})
